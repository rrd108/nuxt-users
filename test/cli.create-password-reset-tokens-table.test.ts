...
describe('CLI: Create Password Reset Tokens Table', () => {
  ...
  afterEach(async () => {
    await cleanupTestSetup(dbType, db, [testOptions.connector!.options.path!], 'password_reset_tokens')
  })
  ...
  it('should create password_reset_tokens table successfully', async () => {
    ...
    const result = await db.sql`SELECT 1 FROM password_reset_tokens LIMIT 1`
  ...
  it('should create table with correct schema', async () => {
    ...
    const result = await db.sql`SELECT id, email, token, created_at FROM password_reset_tokens WHERE email = 'test@example.com'`
  ...
  it('should handle CREATE TABLE IF NOT EXISTS correctly', async () => {
    ...
    const result = await db.sql`SELECT COUNT(*) as count FROM password_reset_tokens`
  ...
  it('should auto-increment ID correctly', async () => {
    ...
    const result = await db.sql`SELECT id, email, token FROM password_reset_tokens ORDER BY id`
  ...
  it('should set default timestamp', async () => {
    ...
    const result = await db.sql`SELECT created_at FROM password_reset_tokens WHERE email = 'test@example.com'`
    const token = result.rows?.[0]
  ...
  it('should allow multiple tokens for same email', async () => {
    ...
    const result = await db.sql`SELECT COUNT(*) as count FROM password_reset_tokens WHERE email = 'same@example.com'`
  ...
  it('should create indexes for performance', async () => {
    ...
    const emailResult = await db.sql`SELECT * FROM password_reset_tokens WHERE email = 'user1@example.com'`
    expect(emailResult.rows).toHaveLength(1)

    const tokenResult = await db.sql`SELECT * FROM password_reset_tokens WHERE token = 'token-2'`
  ...
  it('should handle email field correctly', async () => {
    ...
    const result = await db.sql`SELECT email FROM password_reset_tokens`
  ...
  it('should handle token field correctly', async () => {
    ...
    const result = await db.sql`SELECT token FROM password_reset_tokens`
