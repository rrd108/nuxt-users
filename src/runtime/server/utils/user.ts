...
export const createUser = async (userData: CreateUserParams, options: ModuleOptions): Promise<Omit<User, 'password'>> => {
  ...
  const result = await db.sql`SELECT id, email, name, created_at, updated_at FROM users WHERE email = ${userData.email}` as { rows: Omit<User, 'password'>[] }
...
export const findUserByEmail = async (email: string, options: ModuleOptions): Promise<User | null> => {
  ...
  const result = await db.sql`SELECT * FROM users WHERE email = ${email}` as { rows: User[] }
